# Maximo Work Orders Polymer Sample Application

Use the Polymer sample to learn how you can use Maximo application programming interface (APIs) with a web application. This sample uses APIs to perform work order operations, as an example of how to integrate with Maximo.
...
- [Polymer 2](https://www.polymer-project.org/) (see these instructions [here](https://www.polymer-project.org/2.0/start/install-2-0)).
...
This sample application was built and tested using Google Chrome Version 63.

### How to Define Values to System Properties
  
To define system properties, log in to Maximo and go to *System Properties* application (Open the main menu and select `System Configuration` > `Platform Configuration` > `System Properties`) to open the *System Properties* application. Click in the *Filter* link on top of *Global Properties* table, type `mxe.oslc.aclallow` on *Property Name* field and press *Enter/Return* key. The table will show the CORS related properties. 
  
To change the value of each property, expand the property (click on *View Details* button, a gray right arrow next to the checkbox) and change its *Global Value* field. Perform this for each variable then click the *Save* button on the top of the screen.
...
1. Clone this repository, or download the sample source files.
...
The [with-credentials property](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest/withCredentials) must be set to true in all the requests that are made, so the JavaScript engine can store the authentication session on a cookie and reuse it on subsequent requests. The credentials are invalidated only after they expire or the user logs out. See [src/maximo-login.html](src/maximo-login.html) in the sample code for more information.
...
The logout link on the Sample application redirects to a page that contains the above `iron-ajax` element, and the `auto` attribute forces execution after the page loads. See [src/maximo-logout.html](src/maximo-logout.html) in the sample code for more information.

Sometimes you need to know if the session is still active or not, you can achieve that via the `maximo/oslc/whoami` endpoint:
```html
<iron-ajax
    auto
    with-credentials
...
This mechanism is used in the sample to define if a user is still authenticated after a page reload. See [src/maximo-app.html](src/maximo-app.html) in the sample code for more information.
...
```
The above example specifies that the query will return 100 records per page, with only `wonum,description,name,status,status_description, workorderid` fields in each record. The`pageno` attribute defines the current page, so you can change the page by using the 'Next Page' and 'Previous Page' links. It is necessary to set `collectioncount` in order to get the total number of records returned by this query. See [src/maximo-work-orders-list.html](src/maximo-work-orders-list.html) in the sample code for more information.
